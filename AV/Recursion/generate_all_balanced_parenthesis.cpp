...
Input : n
Let n=2  ===> 1. ()()  2. (())
means we have 4 boxes now we need to put (, (, ), ), in those 4 boxes in such a way finally that should be balanced
...
Here
in starting we can't use closing bracket as that will not make final answer = balanced hence in starting we need to include Open bracket
in short, first box has to be fill by open bracket.

Now O=O-1=3-1 =2
Now next time (for 2nd box) we have two choices either we can use again open bracket or using close bracket .
...
TERMINATION : now here when open finished then apply all close one by one once o=0,c=0 then we have to stop and OP is our final string.

Here we use some observation skills, here sometimes we draw 2 branches , sometimes we draw 1 branch, 
Sometimes we have closing bracket wali branch adn sometimes it is not there, opening bracket is always there till O=0
